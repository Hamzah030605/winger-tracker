...
// keyword → { label, color } for auto-tagging tasks
const TASK_TAGS: { keywords: string[]; label: string; color: string }[] = [
  { keywords: ['linux'],                      label: 'Linux',      color: '#3b82f6' },
  { keywords: ['bash', 'script', 'shell'],    label: 'Bash',       color: '#3b82f6' },
  { keywords: ['docker', 'container'],        label: 'Docker',     color: '#3b82f6' },
  { keywords: ['network', 'networking', 'tcp', 'dns', 'osi'], label: 'Networking', color: '#3b82f6' },
  { keywords: ['kubernetes', 'k8s', 'kubectl', 'helm'],       label: 'K8s',        color: '#3b82f6' },
  { keywords: ['git', 'github', 'commit', 'branch'],          label: 'Git',        color: '#3b82f6' },
  { keywords: ['terraform'],                  label: 'Terraform',  color: '#3b82f6' },
  { keywords: ['aws', 'ec2', 's3', 'vpc'],   label: 'AWS',        color: '#3b82f6' },
  { keywords: ['arabic'],                     label: 'Arabic',     color: '#10b981' },
  { keywords: ['gym', 'lift', 'squat', 'bench', 'deadlift'], label: 'Gym', color: '#f59e0b' },
  { keywords: ['football', 'ball mastery', 'dribble'],        label: 'Football', color: '#10b981' },
]

function detectTag(text: string): { label: string; color: string } | null {
  const lower = text.toLowerCase()
  for (const tag of TASK_TAGS) {
    if (tag.keywords.some(k => lower.includes(k))) return { label: tag.label, color: tag.color }
  }
  return null
}
...
function urgencyColor(days: number) {
  return days > 30 ? '#ff4d1c' : days > 14 ? '#f59e0b' : '#ef4444'
}

function barHue(pct: number) {
  return pct < 60 ? '#ff4d1c' : pct < 80 ? '#f59e0b' : '#ef4444'
}
...
function TopicTag({ label, color }: { label: string; color: string }) {
  return (
    <span
...
function TaskRow({
  task, onToggle, onDelete, onPin, focus = false,
}: {
  task: NoteTask; onToggle: () => void; onDelete: () => void; onPin: () => void; focus?: boolean
}) {
  const tag = detectTag(task.text)
  const bg = task.done ? 'rgba(16,185,129,0.04)' : focus ? 'rgba(255,77,28,0.05)' : 'rgba(255,255,255,0.025)'
  const border = task.done ? '1px solid rgba(16,185,129,0.1)' : focus ? '1px solid rgba(255,77,28,0.12)' : '1px solid rgba(255,255,255,0.05)'

  return (
    <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
      style={{ background: bg, border, opacity: task.done ? 0.55 : 1, transition: 'opacity 0.25s ease' }}>
      {/* Checkbox */}
      <button onClick={onToggle}
        style={{ width: 28, height: 28, minWidth: 28, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          width: 20, height: 20, borderRadius: '50%', border: '2px solid',
          borderColor: task.done ? '#10b981' : focus ? 'rgba(255,77,28,0.5)' : 'rgba(255,255,255,0.2)',
      ...
      {/* Text */}
      <span className="flex-1 text-[13.5px] leading-snug min-w-0 truncate"
        style={{ color: task.done ? 'var(--muted-foreground)' : 'var(--foreground)', textDecoration: task.done ? 'line-through' : 'none' }}>
        {task.text}
      </span>

      {/* Topic tag */}
      {tag && !task.done && <TopicTag label={tag.label} color={tag.color} />}

      {/* Rolled-over indicator */}
      {task.rolledOver && !task.done && (
        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0"
          style={{ background: 'rgba(148,163,184,0.08)', color: 'rgba(148,163,184,0.5)', border: '1px solid rgba(148,163,184,0.15)', letterSpacing: '0.03em' }}>
          ↩
        </span>
      )}
      ...
      <button onClick={onPin}
        ...
        <svg width="13" height="13" viewBox="0 0 24 24"
          fill={task.priority ? '#f59e0b' : 'none'}
          stroke={task.priority ? '#f59e0b' : 'rgba(255,255,255,0.2)'}
          strokeWidth="2">
      ...
      <button onClick={onDelete}
        style={{ width: 28, height: 28, minWidth: 28, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.18)' }}>
...
export function NotesClient({ userId, today, existing, pastNotes, topicRows, planStartDate, rolledOverTasks = [] }: Props) {
  const [tasks, setTasks] = useState<NoteTask[]>((existing?.tasks as NoteTask[] | null) ?? rolledOverTasks)
  // content preserved for streak computation but not shown in UI
  const [content] = useState(existing?.content ?? '')
  ...
  const persist = useCallback(async (t: NoteTask[]) => {
    setSaving(true); setError('')
    const supabase = createClient()
    // Strip rolledOver flag — it's cosmetic only, not persisted
    const clean = t.map(({ rolledOver: _r, ...rest }) => rest)
    const { error: err } = await supabase.from('daily_notes').upsert(
      { user_id: userId, note_date: today, tasks: clean, content: content.trim() || null, updated_at: new Date().toISOString() },
  ...
  function schedule(t: NoteTask[]) {
    if (saveTimer.current) clearTimeout(saveTimer.current)
    saveTimer.current = setTimeout(() => persist(t), 600)
  }

  // Auto-save rolled-over tasks on mount so they persist even without user interaction
  useEffect(() => {
    if (rolledOverTasks.length > 0) persist(rolledOverTasks)
  ...
  function addTask() {
    ...
    setTasks(next); setNewTask(''); schedule(next)
    inputRef.current?.focus()
  }
  function toggleTask(id: string) {
    const next = tasks.map(t => t.id === id ? { ...t, done: !t.done } : t)
    setTasks(next); schedule(next)
  }
  ...
  function pinTask(id: string) {
    ...
    if (!task.priority && tasks.filter(t => t.priority && t.id !== id).length >= 3) return
    ...
    <div className="space-y-3">
        ...
        <div className="px-4 pt-4 pb-0">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right flex-shrink-0 pt-1">
              <p className="font-black leading-none" style={{ fontSize: 40, color: urgencyColor(daysLeft), letterSpacing: '-2px' }}>
                {daysLeft}
              </p>
          ...
          <Label right={
            focusTasks.length > 0
              ? <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                  style={{ background: 'rgba(255,77,28,0.12)', color: 'rgba(255,100,50,0.9)' }}>
                  {focusTasks.length}/3
                </span>
              : tasks.length > 0
              ? <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.2)' }}>Tap ★ to pin</span>
              : null
          }>
            Top 3 Priorities
          </Label>
          ...
          {focusTasks.length === 0 ? (
            <div className="py-3 px-1">
              <p className="text-[13px] font-medium" style={{ color: 'rgba(255,255,255,0.25)' }}>
                Pin your three most critical tasks.
              </p>
              <p className="text-[11px] mt-1 leading-relaxed" style={{ color: 'rgba(255,255,255,0.14)' }}>
                What absolutely must happen today to move closer to G-Research?
              </p>
            </div>
          ) : (
            focusTasks.map(task => (
      ...
      <div className="rounded-2xl overflow-hidden"
            ...
            <div className="flex-1">
              <Label right={
                <span className="text-[10px] font-medium" style={{ color: 'rgba(255,255,255,0.2)' }}>
                  {total === 0 ? '' : `${doneCount} of ${total} done`}
                </span>
              }>
                Today's Tasks
              </Label>
              <div className="h-[2px] rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
                <div className="h-full rounded-full"
                  style={{
                    width: `${progress}%`,
                    background: allDone ? '#10b981' : '#ff4d1c',
        ...
        <div className="px-3 pt-2 pb-2 space-y-1.5">
          {/* Rollover banner */}
          {tasks.some(t => t.rolledOver && !t.done) && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl mb-1"
              ...
              <span className="text-[11px]" style={{ color: 'rgba(148,163,184,0.55)' }}>
                {tasks.filter(t => t.rolledOver && !t.done).length} task{tasks.filter(t => t.rolledOver && !t.done).length !== 1 ? 's' : ''} carried over from yesterday
              </span>
            </div>
          )}
          ...
          {total === 0 && (
            <div className="py-5 text-center px-2">
              <p className="text-[13px] font-medium" style={{ color: 'rgba(255,255,255,0.22)' }}>
                What moves you closer to G-Research today?
              </p>
              <p className="text-[11px] mt-1.5" style={{ color: 'rgba(255,255,255,0.12)' }}>
            ...
            <>
              <div className="flex items-center gap-2 pt-1.5 pb-0.5">
                ...
                <span className="text-[9px] font-semibold uppercase tracking-widest"
                  style={{ color: 'rgba(255,255,255,0.15)' }}>Done</span>
                <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.05)' }} />
              </div>
              {otherDone.map(task => (
                <TaskRow key={task.id} task={task}
          ...
          <div className="flex items-center gap-2 mt-1 px-3 py-2.5 rounded-xl"
            ...
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
              style={{ flexShrink: 0, color: 'rgba(255,255,255,0.18)' }}>
              <path d="M6.5 1v11M1 6.5h11" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
            </svg>
            <input ref={inputRef} type="text" value={newTask}
              onChange={e => setNewTask(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && addTask()}
              placeholder="What gets you closer today?"
      ...
      <div className="rounded-2xl overflow-hidden"
        ...
        <div className="pb-2">
          {ROADMAP_TOPICS.map(({ slug, short }, si) => {
            ...
            const rowColor = pct === 100 ? '#10b981' : doneMods > 0 ? '#3b82f6' : 'rgba(255,255,255,0.18)'
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                        <span className="text-[10px] font-bold" style={{ color: rowColor }}>
                          {doneMods}/{def.totalModules}
                        </span>
                        {pct === 100 && (
                          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                            <circle cx="6" cy="6" r="5.5" fill="rgba(16,185,129,0.15)" stroke="#10b981" strokeWidth="1"/>
                            <path d="M3.5 6l1.8 1.8L8.5 4.5" stroke="#4ade80" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        )}
                      </div>
                    </div>
                    <div className="h-[2px] rounded-full overflow-hidden"
                      ...
                      <div className="h-full rounded-full"
                        style={{ width: `${pct}%`, background: rowColor, transition: 'width 0.7s ease' }} />
                    </div>
                  </div>
                  <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: 15, flexShrink: 0 }}>
                    {isOpen ? '−' : '+'}
                  </span>
                </button>

                {/* Module checklist — smooth accordion */}
                <div style={{
                  maxHeight: isOpen ? `${def.modules.length * 36 + 52}px` : '0px',
                  overflow: 'hidden',
                  ...
                  <div className="px-4 pb-3.5 pt-1 space-y-2"
                      ...
                      return (
                        <div key={idx} className="flex items-start gap-2.5">
                          <div className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0 mt-0.5"
                            style={{ background: done ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.05)' }}>
                            {done ? (
                          ...
                          <span className="text-[12px] leading-snug"
                            style={{ color: done ? 'rgba(74,222,128,0.6)' : 'var(--muted-foreground)' }}>
                            {mod}
                          </span>
                        </div>
                      )
                    ...
                    <Link href={`/devops/topics/${slug}`}
                      className="block pt-1.5 text-[11px] font-semibold"
                      style={{ color: '#3b82f6' }}>
                      Update progress →
                    </Link>
                  </div>
        ...
        <div className="rounded-2xl px-4 pt-3.5 pb-4"
          ...
          {pastNotes.length > 0 && (
            <div className="flex items-end justify-between mb-4">
              {computeHistory(today, pastNotes).map((dot, i) => {
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              <p className="text-[18px] font-bold leading-none" style={{ color: progress > 0 ? '#ff4d1c' : 'var(--muted-foreground)' }}>
                {progress}%
              </p>
              <p className="text-[9px] uppercase tracking-wider mt-1.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                Today
              </p>
            </div>
            <div className="text-center">
              <p className="text-[18px] font-bold leading-none" style={{ color: streak > 0 ? '#fbbf24' : 'var(--muted-foreground)' }}>
                {streak}
              </p>
              <p className="text-[9px] uppercase tracking-wider mt-1.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                🔥 Streak
            ...
            <div className="text-center">
              <p className="text-[18px] font-bold leading-none" style={{ color: best > 0 ? 'rgba(251,191,36,0.7)' : 'var(--muted-foreground)' }}>
                {best}
              </p>
              <p className="text-[9px] uppercase tracking-wider mt-1.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                🏆 Best
      ...
      {saving && (
        <p className="text-[10px] px-1 text-right" style={{ color: 'rgba(255,255,255,0.2)' }}>saving…</p>
      )}
      {error && (
